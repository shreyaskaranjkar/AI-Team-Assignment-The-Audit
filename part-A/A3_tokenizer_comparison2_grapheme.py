from pathlib import Path
import regex
import tiktoken
from transformers import AutoTokenizer

files = {
    "English": "eng_Latn.dev",
    "Hindi": "hin_Deva.dev",
    "Kannada": "kan_Knda.dev",
    "Tamil": "tam_Taml.dev",
}

gpt2 = tiktoken.get_encoding("gpt2")
muril = AutoTokenizer.from_pretrained("google/muril-base-cased")

print("=== A3: TOKENS PER GRAPHEME ===")

for language, filename in files.items():
    path = Path("corpus", filename)
    lines = path.read_text(encoding="utf-8").splitlines()

    total_graphemes = sum(
        len(regex.findall(r"\X", line))
        for line in lines
    )

    gpt2_tokens = sum(
        len(gpt2.encode(line))
        for line in lines
    )

    muril_tokens = sum(
        len(muril.encode(line, add_special_tokens=False))
        for line in lines
    )

    gpt2_tpg = gpt2_tokens / total_graphemes
    muril_tpg = muril_tokens / total_graphemes

    print(f"\n{language}")
    print(f"  Graphemes:         {total_graphemes}")
    print(f"  GPT-2 tokens:      {gpt2_tokens}")
    print(f"  MuRIL tokens:      {muril_tokens}")
    print(f"  GPT-2 tok/grapheme: {gpt2_tpg:.4f}")
    print(f"  MuRIL tok/grapheme: {muril_tpg:.4f}")